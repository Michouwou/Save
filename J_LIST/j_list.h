...
void			jlst_sort(j_list **first_node, int (*compare_func)(void*, void*), int code); /*sorts the list according to 'compare_func', that's not advised because it takes many operations OK*/
...
void			jlst_del_node(j_list **first_node, int index, int code); /*deletes the node at 'index' in the list OK*/
