...
int		ft_type_crossroad(T_LIST *args_d, va_list *args, char **res, int *buf)
{
	if (args_d->type == INT_TYPE)
		return (ft_int_type(args_d, args, res));
	else if (args_d->type == CHAR_TYPE)
		return (ft_char_type(args_d, args, res, buf));
		...
		return (ft_call_wstring(va_arg(*args, wchar_t*), args_d, res));
